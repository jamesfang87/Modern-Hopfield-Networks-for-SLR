...
class LatchSequenceSet(Dataset):
    ...
    def _generate_latch_sequences(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # Generate data and target tensors.
        generated_data = torch.randn(size=(self.__num_samples, self.__num_instances, self.__num_characters))
        generated_signal = torch.randint(low=0, high=10, size=(self.__num_samples,), dtype=torch.int64)
        #generated_signal = torch.nn.functional.one_hot(input=generated_signal, num_classes=10)
